...
public class ADYFX_FlattenTex :EditorWindow
{
    ...
    bool bmula = false;
    bool amula = false;
    private RGBASele rTongDao = RGBASele.此图取R通道;
    private RGBASele gTongDao = RGBASele.此图取R通道;
    private RGBASele bTongDao = RGBASele.此图取R通道;
    private RGBASele aTongDao = RGBASele.此图取R通道;
    private string path = ">>还未选择保存路径";
    private int serial = 1;
    private string tex2dName = "ADYFX_PinHe";
    private Texture2D toumingtex;
    ...
    private Shader btnhoverShader;//绘制按钮悬停特效的shader
    private Material btnhoverMat;
    private Texture2D txglowtex;//帮助特效流光
    private Texture2D txglowmasktex;//帮助特效遮罩
    private Texture2D sourceTex0;
    private Texture2D sourceTex1;
    private Texture2D sourceTex2;
    private Texture2D sourceTex3;

    private Texture2D sourceTex4;
    RenderTexture rt;//绘制到RT
    RenderTexture rt1;
    RenderTexture rt2;
    private Texture sele01YuLan;
    ...
    private Vector2 resolution1 = new Vector2(512, 512);
    bool isAlpha = false;
    bool isHeiBai = false;
    bool isQuSe = false;
    [MenuItem("ADYFX/贴图工具/※拼合贴图到通道", false, 3001)]
    ...
    private void Sele01()
    {
        ...
        if (ADYFX_Editor.IsShuBiaoPos(new Vector4(1062, 1090, 194, 223), myMousePosition))//设置尺寸按钮 事件
        {
            ...
            {
                resolution = ADYFX_Editor.GetTexWH(ADYFX_Editor.GetPath(sourceTex0));
            }
        ...
        if (ADYFX_Editor.IsShuBiaoPos(new Vector4(1095, 1124, 194, 225), myMousePosition))//设置尺寸按钮 事件
        {
            ...
            if (Event.current.type == EventType.MouseDown && sourceTex1 != null) 
                resolution = ADYFX_Editor.GetTexWH(ADYFX_Editor.GetPath(sourceTex1));
        }
        if (ADYFX_Editor.IsShuBiaoPos(new Vector4(1129, 1157, 194, 225), myMousePosition))//设置尺寸按钮 事件
        {
            ...
            if (Event.current.type == EventType.MouseDown && sourceTex2 != null) 
                resolution = ADYFX_Editor.GetTexWH(ADYFX_Editor.GetPath(sourceTex2));
        }
    ...
    {
        bantoumbtn = ADYFX_Editor.GetTex_AssetColor("eed0fe183bf73d04da6003095000fd0f");
        rgbaSelseR = ADYFX_Editor.GetTex_AssetColor("0f6e4916a04c06d41877198d6549e946");
        rgbaSelseG = ADYFX_Editor.GetTex_AssetColor("c5ec1d9fe6f408e468bb1af4e51f2768");
        rgbaSelseB = ADYFX_Editor.GetTex_AssetColor("f88586f9f20424f499f16a98a26d2f1b");
        rgbaSelseA = ADYFX_Editor.GetTex_AssetColor("3453eb1b84f48164dbd794909d9f07c9");
        boxbooltex = ADYFX_Editor.GetTex_AssetColor("0c28ce8f8a36fa34590dbd1e14bf16d4");
        boxbooltex1 = ADYFX_Editor.GetTex_AssetColor("35cff779851180b4cbf4759550a4cf65");
        BGtexture01 = ADYFX_Editor.GetTex_AssetColor("68898fbad48dce3458d3aef1b42f7005");
        helptex = ADYFX_Editor.GetTex_AssetColor("dcb1d2d4a1a3a2d48aef51d4cad53a8e");
        txglowtex = ADYFX_Editor.GetTex_AssetColor("9c894082522d9e04b968dd538d83e82e");
        txglowmasktex = ADYFX_Editor.GetTex_AssetColor("32bf3c913e9b43d4aa306cdc4fbab346");
        selsetex = ADYFX_Editor.GetTex_AssetColor("8d76ab80da83f614fba7b6c6cc066408");
        ...
        toumingtex = ADYFX_Editor.GetTex_AssetColor("d3963aed91cf5034bb22a31b529891ad");
    ...
    void FlattenImage1() 
    {
        Texture2D texr = new Texture2D((int)resolution.x, (int)resolution.y, TextureFormat.ARGB32, false);
        Texture2D texg = new Texture2D((int)resolution.x, (int)resolution.y, TextureFormat.ARGB32, false);
        Texture2D texb = new Texture2D((int)resolution.x, (int)resolution.y, TextureFormat.ARGB32, false);
        Texture2D texa = new Texture2D((int)resolution.x, (int)resolution.y, TextureFormat.ARGB32, false);
        byte[] byter;//生命一个字节文件
        byte[] byteg;//生命一个字节文件
        byte[] byteb;//生命一个字节文件
        byte[] bytea;//生命一个字节文件
        if (sourceTex0)
        {
            Vector2 texrwh = ADYFX_Editor.GetTexWH(ADYFX_Editor.GetPath(sourceTex0));
            texr.width = (int)texrwh.x;
            texr.height = (int)texrwh.y;
            byter = File.ReadAllBytes(ADYFX_Editor.GetPath(sourceTex0));
            texr.LoadImage(byter);
            texr.Apply();
            if (texr.width!= resolution.x && texr.height != resolution.y)
            {
                colorr = new Color[(int)resolution.x * (int)resolution.y];
                colorr = ADYFX_Editor.TextureScale(texr, (int)resolution.x, (int)resolution.y).GetPixels();
            }
        }
        if (sourceTex1)
        {
            Vector2 texgwh = ADYFX_Editor.GetTexWH(ADYFX_Editor.GetPath(sourceTex1));
            texg.width = (int)texgwh.x;
            texg.height = (int)texgwh.y;
            byteg = File.ReadAllBytes(ADYFX_Editor.GetPath(sourceTex1));
            texg.LoadImage(byteg);
            texg.Apply();
            if (texg.width != resolution.x && texg.height != resolution.y)
            {
                colorg = new Color[(int)resolution.x * (int)resolution.y];
                colorg = ADYFX_Editor.TextureScale(texg, (int)resolution.x, (int)resolution.y).GetPixels();
            }
        }
        if (sourceTex2)
        {
            Vector2 texbwh = ADYFX_Editor.GetTexWH(ADYFX_Editor.GetPath(sourceTex2));
            texb.width = (int)texbwh.x;
            texb.height = (int)texbwh.y;
            byteb = File.ReadAllBytes(ADYFX_Editor.GetPath(sourceTex2));
            texb.LoadImage(byteb);
            texb.Apply();
            if (texb.width != resolution.x && texb.height != resolution.y)
            {
                colorb = new Color[(int)resolution.x * (int)resolution.y];
                colorb = ADYFX_Editor.TextureScale(texb, (int)resolution.x, (int)resolution.y).GetPixels();
            }
        }
        if (sourceTex3)
        {
            Vector2 texawh = ADYFX_Editor.GetTexWH(ADYFX_Editor.GetPath(sourceTex3));
            texa.width = (int)texawh.x;
            texa.height = (int)texawh.y;
            bytea = File.ReadAllBytes(ADYFX_Editor.GetPath(sourceTex3));
            texa.LoadImage(bytea);
            texa.Apply();
            if (texa.width != resolution.x && texa.height != resolution.y)
            {
                colora = new Color[(int)resolution.x * (int)resolution.y];
                colora = ADYFX_Editor.TextureScale(texa, (int)resolution.x, (int)resolution.y).GetPixels();
            }
        }
        /////
