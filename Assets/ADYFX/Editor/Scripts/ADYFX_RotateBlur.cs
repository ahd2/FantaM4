...
public class ADYFX_RotateBlur : EditorWindow
{
    ...
    [MenuItem("ADYFX/贴图滤镜/※旋转模糊", false, 3510)]
    ...
    private void OnGUI() 
    {
        ...
        if (myMousePosition.x > 738 && myMousePosition.x < 838 && myMousePosition.y > 287 && myMousePosition.y < 317)
        {
            if (Event.current.type == EventType.MouseDown ) 
            {
                ...
                {
                    resolution = ADYFX_Editor.GetTexWH(ADYFX_Editor.GetPath(sourceTex));
                }    
            }
    ...
    void GSBlurSave()
    {
        if (sourceTex != null)
        {
            Texture2D YYY = ADYFX_Editor.GetYuanTu(ADYFX_Editor.GetPath(sourceTex));
    ...
    {
        bgtex = ADYFX_Editor.GetTex_AssetColor("7568448cae60c9042a4759d35380c54b");
        noneyulantex = ADYFX_Editor.GetTex_AssetColor("6d09b5ae1be5836469ac7ef7379b098e");
        noneyuantutex = ADYFX_Editor.GetTex_AssetColor("f09cd75235d26734c823cfcec21356d0");
        silidertex = ADYFX_Editor.GetTex_AssetColor("35cff779851180b4cbf4759550a4cf65");
        silidertex1 = ADYFX_Editor.GetTex_AssetColor("bbe8d7e12ad163340aac37afa8545747");
        ...
        btnmain = ADYFX_Editor.GetTex_AssetColor("e4c2fc66ca9406741aa3ec379a81bc64");
