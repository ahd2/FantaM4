...
public class ADYFX_Tex2DPixelSize : EditorWindow
{
    ...
    [MenuItem("ADYFX/贴图工具/※修改贴图大小（单张）", false, 3031)]
    ...
    private void OnGUI()
    {
        ...
        sourceTex0 = EditorGUILayout.ObjectField(sourceTex0, typeof(Texture), false, options) as Texture2D;//然后声明这个tex2d
                                                                                                           //dx = EditorGUILayout.ObjectField(dx, typeof(Mesh), false, options) as Mesh;//然后声明这个tex2d
    ...
    void Save()
    {
        ...
        t2 = ADYFX_Editor.GetYuanTu(ADYFX_Editor.GetPath(sourceTex0));
