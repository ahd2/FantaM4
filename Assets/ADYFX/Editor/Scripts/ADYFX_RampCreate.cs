...
public class AFX_RampCreate : EditorWindow
{
    ...
    [MenuItem("ADYFX/贴图工具/※渐变图生成工具", false, 3012)]
    static void RampCreateWindow ()//菜单窗口
    {
        ...
        window.position = new Rect(1500,250,350,600);
        window.titleContent = new GUIContent("渐变图生成工具");//标题内容
        window.Show();//创建窗口
    }
    private void OnEnable()
    {
        assets = ADYFX_Editor.GetOBJ("de325a491d304224e8d012d90ccbd981", true) as ADYFX_Common_Assets;
        path = assets.strs1[1];
        tex2dName = assets.strs1[0];
        Debug.Log(path);
        Debug.Log(tex2dName);
    ...
    {
        string temppath = AssetDatabase. GUIDToAssetPath("de325a491d304224e8d012d90ccbd981");
        AssetDatabase.DeleteAsset(temppath); //关闭窗口时删除旧配置（重新生成配置文件，否则关闭引擎将丢失此次打开引擎之后的修改）
        ADYFX_Common_Assets level = ScriptableObject.CreateInstance<ADYFX_Common_Assets>();//不刷新库 创建新的配置以继承旧配置的guid
        level.strs1 = new List<string>();
        level.strs1.Add( tex2dName);
        level.strs1.Add(path);
        AssetDatabase.CreateAsset(level, temppath);
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("查看使用教程（视频）"))
            Application.OpenURL("https://space.bilibili.com/7234711/channel/seriesdetail?sid=613948&ctype=0");
    }
    void OutRampTex()//输出
    {
    ...
    void Save(Color[] colors)//写入像素数据并保存到指定绝对路径
    {
        ...
        if (isAlpha)
        {
            _texFormat = TextureFormat.RGBA32;//贴图设置32位 有A通道
        }
        else
        {
