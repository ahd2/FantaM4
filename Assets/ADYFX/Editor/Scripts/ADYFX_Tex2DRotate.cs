...
public class ADYFX_Tex2DRotate : Editor
{
    ...
    static void Zhengjiushi()
    {
        ...
        //string path1 = ADYFX_Editor.GetPath(Selection.objects[0]);
        //Texture2D yuantu = ADYFX_Editor.GetYuanTu(path1);
        ...
        Texture2D rotateTex = new Texture2D(yuantu.height, yuantu.width, TextureFormat.RGBA32, false);//创建一个新tex2d
    ...
    static void YiBaiBashi()
    {
        ...
        maxcishu -= 1;
        Texture2D rotateTex = new Texture2D(yuantu.width, yuantu.height, TextureFormat.RGBA32, false);//创建一个新tex2d
        int cishu = 0;//记录已循环的次数 set对应序号到颜色组
        for (int i = 0; i < yuantu.width; i++)//宽高循环 贴图旋转九十度
        {
            for (int j = 0; j < yuantu.height; j++)//每次宽度循环都 循环一次高度
    ...
    static void Save(Color[] colors, Vector2 kuangao, string name1, string path1, TextureImporter ttt ,int geshi)//写入像素数据并保存到指定绝对路径
    {
        ...
        {
            texformat = TextureFormat.RGBA32;//贴图设置32位 有A通道
        }
        else if (geshi == 2)
        {
            texformat = TextureFormat.RGBA32;//贴图设置32位 有A通道
