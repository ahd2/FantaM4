...
public class ADYFX_GaussianBlur_Window : EditorWindow
{
    ...
    public string 窗口背景图路径 = "62054b429fa5665418fabdc628c00d31";
    ...
    public string btn01tex0path = "d989713299c14d04fab66a8290a24a31";
    public Texture2D btn01tex1;//按钮图悬停
    public string btn01tex1path = "fe33e3ad6370dbd4db366233b97fc112";
    private Material btn01mat;
    Material wanggemat;
    public Texture2D btn02tex0;//按钮图待机
    public string btn02tex0path = "dbf9e660ba954ad46b38f40cd0a5c798";
    public Texture2D btn02tex1;//按钮图悬停
    public string btn02tex1path = "39c1b14322941414ba9c7cb90fb6c2e0";

    public Texture2D btn03tex0;//按钮图待机
    public string btn03tex0path = "bff9d343dda0ad74687f171d768f8f51";
    public Texture2D btn03tex1;//按钮图悬停
    public string btn03tex1path = "4204b54ea10a0d54082a102f551dbe64";

    public Texture2D btn04tex0;//按钮图待机
    public string btn04tex0path = "48f1bc0b06914df4ba1c33536b17385f";
    public Texture2D btn04tex1;//按钮图悬停
    public string btn04tex1path = "5d98fd95163a3f34db9c850a10f856d9";

    public Texture2D btnsavetex0;//按钮图待机
    public string btnsavetex0path = "f6ec4812b0f3d2340a41196377e643ec";
    public Texture2D btnsavetex1;//按钮图悬停
    public string btnsavetex1path = "3edf482c53149a74fb702440ac2ebfe3";

    public Texture2D btnoenpathtex0;//按钮图待机
    public string btnoenpathtex0path = "ae73ceda11855c549978e2d37e210e09";
    public Texture2D btnoenpathtex1;//按钮图悬停
    public string btnoenpathtex1path = "b495c873f89410942bd5c261d644bfc1";
    ...
    [MenuItem("ADYFX/贴图滤镜/※高斯模糊", false, 3502)]
    ...
    void StartWindow() //初始化窗口 获取所需资源
    {
        ...
        {
            窗口背景图 = ADYFX_Editor.GetTex_AssetColor(窗口背景图路径);
            btn01tex0 = ADYFX_Editor.GetTex_AssetColor(btn01tex0path);
            btn01tex1 = ADYFX_Editor.GetTex_AssetColor(btn01tex1path);
            wanggemat = new Material(Shader.Find("ADYFX/Editer/PngWangGe"));
            btn02tex0 = ADYFX_Editor.GetTex_AssetColor(btn02tex0path);
            btn02tex1 = ADYFX_Editor.GetTex_AssetColor(btn02tex1path);

            btn03tex0 = ADYFX_Editor.GetTex_AssetColor(btn03tex0path);
            btn03tex1 = ADYFX_Editor.GetTex_AssetColor(btn03tex1path);

            btn04tex0 = ADYFX_Editor.GetTex_AssetColor(btn04tex0path);
            btn04tex1 = ADYFX_Editor.GetTex_AssetColor(btn04tex1path);

            btnsavetex0 = ADYFX_Editor.GetTex_AssetColor(btnsavetex0path);
            btnsavetex1 = ADYFX_Editor.GetTex_AssetColor(btnsavetex1path);

            btnoenpathtex0 = ADYFX_Editor.GetTex_AssetColor(btnoenpathtex0path);
            btnoenpathtex1 = ADYFX_Editor.GetTex_AssetColor(btnoenpathtex1path);

            特效遮罩图1 = ADYFX_Editor.GetTex_AssetColor("e4c2fc66ca9406741aa3ec379a81bc64");
            ...
            特效遮罩图0 = ADYFX_Editor.GetTex_AssetColor("7cb58b66787eac94eb1c964a11cdf009");
            特效发光图0 = ADYFX_Editor.GetTex_AssetColor("e09617715e5620a43a6f1d7d58e0eaf5");
    ...
    private void OnGUI()
    {
            ...
            EditorGUI.DrawPreviewTexture(new Rect(720, 225, 365, 65), btn01tex1, btn01mat);
                ...
                if (原图 != null) 
                {
                  //  resolution = new Vector2(原图.width, 原图.height);
                    resolution = ADYFX_Editor.GetTexWH(ADYFX_Editor.GetPath(原图));
                }
       ...
       //float的一个办法 public static float FloatField(string label, float value, GUIStyle style, params GUILayoutOption[] options);
        ...
        GUI.Label(new Rect(0, 625, 500, 25), "如果贴图的设置maxSize小于原图尺寸  只会影响预览精度   执行保存时会使用贴图原始尺寸来模糊");
    ...
    void GSBlur()
    {
        if (原图 != null)
        {
            ...
            Texture2D texture2D = new Texture2D(rt.width, rt.height, TextureFormat.ARGB32, false);//, TextureFormat.RGBAFloat, false
    ...
    void GSBlurSave()
    {
        if (原图 != null)
        {
            Texture2D YYY = ADYFX_Editor.GetYuanTu(ADYFX_Editor.GetPath(原图));
            ...
            GL.LoadPixelMatrix(0, rt.width, rt.height, 0);//GL.设置像素参数
            Rect rect = new Rect(0, 0, rt.width, rt.height);//定义一个矩形  位置 宽高
            Graphics.DrawTexture(rect, YYY, addmat);//渲染.绘制tex，参数传入矩形和要绘制进去的贴图
            Material mat1;
            mat1 = new Material(gsblur);
            mat1.SetFloat("_BlurSpread", blurScale * 0.01f);
            mat1.SetVector("_offset1", new Vector4(1, 0, 0, 0));
