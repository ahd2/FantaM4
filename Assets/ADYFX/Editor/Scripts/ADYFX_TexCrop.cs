...
public class ADYFX_TexCrop : EditorWindow
{
    ...
    [MenuItem("ADYFX/贴图工具/※剪裁贴图", false, 3040)]
    ...
    private void OnGUI()
    {
        ...
        if (sourceTex0!=null)//判断资源变动  改变当前处理 文字
        {
            ...
            //Texture2D YYY = ADYFX_Editor.GetYuanTu(ADYFX_Editor.GetPath(sourceTex0));
            //EditorGUI.DrawPreviewTexture(yulanRect, YYY, yulanmat);
        ...
        if (GUI.Button(new Rect(1640, 245, 120, 25), "自动计算"))
        {
            if (sourceTex0)
            {
                //Vector2 tt = ADYFX_Editor.GetTexWH(ADYFX_Editor.GetPath(sourceTex0));
                float uu = (sourceTex0.width * uvscale) * caijianquyu.z;
                float vv = (sourceTex0.height * uvscale) * caijianquyu.w;
                //float uu = (tt.x * uvscale) * caijianquyu.z;
                //float vv = (tt.y * uvscale) * caijianquyu.w;
                uu = (int)uu;
                vv = (int)vv;
    ...
    void StartWindow()
    {
        touminggezi = ADYFX_Editor.GetTex_AssetColor("8d3d4f144172a0149ba6fcc445ccece6");
        ...
        bgtex = ADYFX_Editor.GetTex_AssetColor("4352a397a95da5b42a19c03f75f6e17c");
        jiancaikuang0 = ADYFX_Editor.GetTex_AssetColor("150a04e9d3eb3ed4a965631fbed7bcb6");
        jiancaikuang1 = ADYFX_Editor.GetTex_AssetColor("fd777b8ca4385c8478ecfe512783eead");
        jiancaikuang2 = ADYFX_Editor.GetTex_AssetColor("1f03fccff54252a44b74bfa8d030905a");
        jiancaikuang3 = ADYFX_Editor.GetTex_AssetColor("7d1256259501ff6498d5819925630694");
        jiancaikuangline = ADYFX_Editor.GetTex_AssetColor("eed0fe183bf73d04da6003095000fd0f");
    }
