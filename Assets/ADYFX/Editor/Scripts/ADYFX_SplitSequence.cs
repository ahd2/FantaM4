...
public class ADYFX_SplitSequence : EditorWindow
{
    ...
    Color[] colorall;
    [MenuItem("ADYFX/贴图工具/※拆分序列图", false, 3018)]
    ...
    void StartWindow() //初始化窗口 获取所需资源
    {
        ...
        boxtex = ADYFX_Editor.GetTex_AssetColor("5f30f57bfce4af14b8b1b9c2d79cca62");
        ...
        gouxuan = ADYFX_Editor.GetTex_AssetColor("84a2fce7f4a47c3499ee0370462cfc3c");
    }
    ...
    void Moshi1()
    {
        ...
        if (GUI.Button(new Rect(890, 245, 100, 25), "自动计算"))
        {
            if (yuantu) 
            {
                Vector2 tt = ADYFX_Editor.GetTexWH(ADYFX_Editor.GetPath(yuantu));
                resolution = new Vector2(tt.x / xulieW, tt.y / xulieH);
            }
    ...
    void Save()
    {
        if (yuantu != null)
        {
            Texture2D YYY = ADYFX_Editor.GetYuanTu(ADYFX_Editor.GetPath(yuantu));
            ...
            for (int i = 0;i <xulieW*xulieH;i++)
            {
               ...
               //Graphics.DrawTexture(rect, yuantu, addmat);//渲染.绘制tex，参数传入矩形和要绘制进去的贴图
                Graphics.DrawTexture(new Rect(0, 0, rt.width, rt.height), YYY, caifenSavemat);//渲染.绘制tex，参数传入矩形和要绘制进去的贴图
    ...
    void Save1()
    {
        if (yuantu != null)
        {
            ...
            caifenSavemat.SetInt("_H", xulieH);
                ...
                Graphics.DrawTexture(new Rect(0, 0, rt.width, rt.height), YYY, caifenSavemat);//渲染.绘制tex，参数传入矩形和要绘制进去的贴图
                GL.PopMatrix();//GL.保存矩阵
                Texture2D texture2D = new Texture2D(rt.width, rt.height, TextureFormat.RGBA32, false);//, TextureFormat.RGBAFloat, false
                texture2D.ReadPixels(new Rect(0, 0, rt.width, rt.height), 0, 0);
                texture2D.Apply();
                colorall = new Color[(int)(texture2D.width * texture2D.height)];//初始化颜色数组长度为出图宽高
                colorall = texture2D.GetPixels();
