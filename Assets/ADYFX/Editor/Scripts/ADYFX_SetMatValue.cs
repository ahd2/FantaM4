...
public class ADYFX_SetMatValue : EditorWindow
{
    ...
    public float qishishengxiaoceng = 1;
    ...
    public string valuename = "_Emission";
    ...
    [MenuItem("ADYFX/特效辅助/※批量修改材质Value", false, 2111)]
    static void RadialblurWindowcus()//菜单窗口
    {
        ADYFX_SetMatValue window = EditorWindow.GetWindow<ADYFX_SetMatValue>();//定义窗口类
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Label("按判断规则来修改材质Value", style);
        ...
        yunshuan = (Yunsuan)GUILayout.Toolbar((int)yunshuan, new string[] { "原Value"+"+" + value, "原Value" + "-" + value, "原Value" + "*" + value, "原Value" + "=" + value }, GUILayout.Width(500));//根据toober单选 改变枚举值 从而影响下面判断  运行不同的功能函数
    ...
    void ZhiXing() 
    {
        for (int i = 0; i < mats.Count; i++) 
        {
            if (iftype == Iftype.大于) 
            {
                if (mats[i].GetFloat(valuename) > qishishengxiaoceng) 
                {
                    if (yunshuan == Yunsuan.加) 
                    {
                        mats[i].SetFloat(valuename, mats[i].GetFloat(valuename) + value);
                    }
                    if (yunshuan == Yunsuan.减)
                    {
                        mats[i].SetFloat(valuename, mats[i].GetFloat(valuename) - value);
                    }
                    if (yunshuan == Yunsuan.乘)
                    {
                        mats[i].SetFloat(valuename, mats[i].GetFloat(valuename) * value);
                    }
                    if (yunshuan == Yunsuan.等于)
                    {
                        mats[i].SetFloat(valuename,  value);
                    }
                }
            }
